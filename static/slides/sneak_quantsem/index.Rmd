...
- y axis is relative error, i have set up panels with the same numbers as in greenboxes
- at first i'm plotting OM, which of course is gold standard, not misspeced and therefore the same each time
...
- we run our misspecified EMs and plot on top
- we'd expect to see that with increasing numbers of misspecs, the error gets larger
- aka the green line not so much like the white
- there's a simple way of looking at this, which is instead of plotting raw data, just 
- calc the relative error in each yeaer and plot that
---
...
- the white line at zero is now representin gthe oM
- so this is what we'd expect to see, and frankly the idea that models do better with 
- less things wrong isn't exaclty a shocker...
...
???
- which is why we are especially interested in this second question, all about IDENTITY
- recall that each EM has a correction, and that enables us to detect what happened when something was corrected
- again this is fake data. the way this plot works is the x axis is the IDENTITY of a corrected model component
- and the y is the DECLINE in relative error, in other words how much the model improved once we corrected that
- in this fake example, X/Y seem to give the biggest bang for buck
---
...
???
- Folks could probably do this in one weekend
...
```{r echo = FALSE, fig.align = 'left', out.height="5%", out.width="5%",fig.retina = 2}
knitr::include_graphics("legend3.png")
```
<Br><Br><br><br><Br><Br><Br><Br>
<Br><Br><br><br><Br><Br><br><br>
```{r echo = FALSE, fig.align = 'left', out.height="15%", out.width="25%",fig.retina = 2}
