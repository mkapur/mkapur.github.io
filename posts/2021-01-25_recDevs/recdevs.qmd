...
Consider a random variable X that follows a lognormal distribution:
$X \sim Lognormal(μ, σ)$ 
...
The coefficient of variation (CV) on the original scale is defined as the ratio of the standard deviation to the mean of the random variable X:
$CV_{original} = \frac{σ_X}{μ_X}$
