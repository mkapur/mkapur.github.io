...
Where $l$ is the length bin in question, $\tilde l$ is the observed length-at-age, and  $\sigma$ is the variation in growth.
...
In pictures: let's say we are interested in $a = 5$. We are going to multiply each point on the length-selectivity by its corresponding point on the probability of length-at-age-5 plot, then sum the results. 
```{r, echo = FALSE}
ggplot(data.frame(cbind(length_selex,length_bins)),
       aes(x = length_bins, y = length_selex)) +
  theme(legend.position = 'none')+
  scale_color_grey() +
  geom_point(aes(x = length_bins, y = length_selex, col = factor(length_bins)))+ 
...
ggplot() +
  ...
  theme(legend.position = 'none')+
  scale_color_grey() + 
  geom_point(data = subset(longPal, age_bins ==5), 
             aes(x = len, y = value, col = factor(len) )) +
  geom_vline(xintercept = 28, lty = 'dotted')
```

In code (still using $a = 5$ as an example):
```{r message = FALSE, warning = FALSE}
# longPal %>% filter(age_bins == 5) %>% select(age_bins, len, value)
...
ggplot(sel_age,
  ...
  geom_line(lwd = 1.1, col = 'seagreen4') +
